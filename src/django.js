...
exports.django = async (projectDir, projectName) => {
  ...
  if(shell.exec( `${sed} -i  "1s|^|CORS_ORIGIN_ALLOW_ALL=True\\n|" settings.py`).code != 0) {
      console.log(chalk.red("Error: failed to add hello_world to list of installed apps"));
      shell.exit(1);
    }

  if(shell.exec( `${sed} -i  "s|'django.contrib.staticfiles' ,|'django.contrib.staticfiles',\\n'hello_world',\\n 'corsheaders',|" settings.py`).code != 0) {
      console.log(chalk.red("Error: failed to add hello_world to list of installed apps"));
      shell.exit(1);
    }
  ...
  if(shell.exec( `${sed} -i  "s|'django.middleware.security.SecurityMiddleware',|'corsheaders.middleware.CorsMiddleware',\\n'django.middleware.common.CommonMiddleware',\\n'django.middleware.security.SecurityMiddleware',\\n|" settings.py`).code != 0) {
      console.log(chalk.red("Error: failed to add hello_world to list of installed apps"));
      shell.exit(1);
  }

  console.log(chalk.magenta("copying views.py and urls.py"));
  if(shell.cp("-r", `${working_dir}/templates/django/*.py`, `${projectDir}/djangoServer/hello_world`).code != 0) {
  ...
  if(shell.exec(`${sed} -i "s|from django.urls import path|from django.urls import path, include|g" urls.py`).code != 0) {
      console.log(chalk.red("Error: failed to add hello_world to list of installed apps"));
      shell.exit(1);
    }
  //add path('', include('hello_world.urls')),
  console.log(chalk.magenta("Edditing django.urls 2/2"));
  if(shell.exec(`${sed} -i  "s|path('admin/', admin.site.urls),|path('admin/', admin.site.urls), path('', include('hello_world.urls')),|g" urls.py`).code != 0) {
      console.log(chalk.red("Error: failed to add hello_world to list of installed apps"));
      shell.exit(1);
    }
